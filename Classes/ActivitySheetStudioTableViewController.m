...
#import "ActivitySheetStudioTableViewController.h"
#import "CreateActivityViewController.h"

static sqlite3 *database = nil;

@implementation ActivitySheetStudioTableViewController
...
- (void)viewDidLoad {
	...
	[fileArray addObject:@"Create a New Activity Sheet"];
	
	NSLog(@"Testing DB");
	NSString * dbPath = [self getDBPath];
	NSLog(@"Path is %@",dbPath);
	
	const char *sql = "select distinct fName from ActivitySheets";
...
{
	NSLog(@"Removing Activity Sheet %@",name);
	NSLog(@"Testing DB");
	NSString * dbPath = [self getDBPath];
	NSLog(@"Path is %@",dbPath);
	...
	[NSMutableString stringWithString:
	 @"delete from ActivitySheets where fName = "];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	appDelegate.viewController = [[CreateActivityViewController alloc] initWithNibName:@"CreateActivityViewController" bundle:nil];
	NSLog(@"test");
	...
	CreateActivityViewController *createActivityViewController = appDelegate.viewController;//[[CreateActivityViewController alloc] initWithNibName:@"CreateActivityViewController" bundle:nil];
	//[createActivityViewController init:@"":1];

	if(indexPath.row == 0)
	{
		NSLog(@"son of a bitch...");
		[createActivityViewController init:@"":1:fileArray];
	}
	else
	{
		NSLog(@"sons of a bitches...");
		[createActivityViewController init:[fileArray objectAtIndex:indexPath.row]:0:fileArray];
	}
	...
	[createActivityViewController release];
