...
@implementation CreateCoinViewController
			...
			for(int i = 0; i <=15; i++)
			{
				...
				if(i+1 > numIcons && lineID[i][0] != 0)
				{
					...
					[NSMutableString stringWithString:
					 @"delete from CoinSheets where lineID = "];
				...
				else if(i+1 > numIconsOld && numIcons > numIconsOld && i+1 <= numIcons)
				{
					...
					NSMutableString *nssql = 
					[NSMutableString stringWithString:
					 @"insert into CoinSheets(fName,imName,auName,auEXT,price) Values("];
					[nssql appendString:[NSString stringWithFormat:@"%@,\"%@\",\"%@\",\"%@\",\"%@\")",editsheet,imName[i],auName[i],auExt[i],price[i]]];
					const char *sql = [nssql UTF8String];
				...
				{
					NSMutableString *nssql =
					[NSMutableString stringWithFormat:@"update CoinSheets set imName=\"%@\",auName=\"%@\",auExt=\"%@\",price=\"%@\" where lineID=%d",
					 imName[i],auName[i],auExt[i],price[i],lineID[i][0]];
					const char *sql = [nssql UTF8String];
...
- (void)viewDidLoad {
	...
	if(dirty == 0)
	{
		...
		if (sqlite3_open([dbPath UTF8String], &database) == SQLITE_OK) 
		{
			...
			NSMutableString *nssql = 
			[NSMutableString stringWithString:
			 @"select lineID,imName,auName,auEXT,price from CoinSheets where fName = "];
			...
			if(sqlite3_prepare_v2(database, sql, -1, &selectstmt, NULL) == SQLITE_OK) 
			{
				...
				while(sqlite3_step(selectstmt) == SQLITE_ROW) {
					...
					NSString *temp_price = [NSString stringWithUTF8String:(char *)sqlite3_column_text(selectstmt, 4)];
					...
					price[i]=[temp_price mutableCopy];
...
//        [alert release];
	...
	for (int i=0; i<=15; i++) 
	{
		...
		LabelArr[i].text = price[i];
		LabelArr[i].textAlignment = UITextAlignmentCenter;
		int label_y;
		label_y = [fArray getVal:i :1]+[fArray getVal:i :3];
		LabelArr[i].font = [UIFont fontWithName:@"Helvetica" size: 12];//used to be 30
		
		//Used for starting with 3 buttons
    ...
    //Edited out to maintain 16 buttons to fit background image
...
- (IBAction) goBack{
	selectAlert = 4;
	UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Do you want to leave the editor?" message:@"All unsaved changes will be lost."
...
- (void) imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    /////Added by nick create copy of imported image and stored in apps memory to be used when making activity sheets
   ...
   UIGraphicsEndImageContext();
    ...
    [UIImageJPEGRepresentation(newImage, 0.00000000000000000000000000000000000000000000000001) writeToFile:jpgPath atomically:YES];
...
-(void)getFileName{
	...
	UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Please Enter Information\n" message:@""
												   delegate:self cancelButtonTitle:@"Cancel"  otherButtonTitles:@"Submit", nil];
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    ...
    if (buttonIndex == 1 && selectAlert == 1) {
		...
		[UIImageJPEGRepresentation(img, 0.00000000000000000000000000000000000000000000000001) writeToFile:jpgPath atomically:YES];
        CGRect rect = CGRectMake(0.0, 0.0, 50.0, 50.0);
        UIGraphicsBeginImageContext(rect.size);
      //  [image drawInRect:rect];
      //  NSFileManager *fileMgr = [NSFileManager defaultManager];
	...
	{
		NSLog(@"Create Item Select Sheet -- Item Select Sheet Name Submit");
		if (textfieldName.text.length == 0)
		{
			NSLog(@"Item Select Sheet name is blank");
...
//        [alert release];
	...
	{
		NSLog(@"Create Item Select Sheet -- Item Select Sheet Name Cancelled");
	...
	else if (buttonIndex == 1 && selectAlert == 3)
	{
		...
		price[tempID] = [textfieldName.text mutableCopy];
		lineID[tempID][1] = 1;
		
	}
	else if(buttonIndex == 1 && selectAlert == 4)
	{
...
- (IBAction)chooseEdit:(id)sender{
	...
	//-----End Audio Button----
	//Omitted for lack of audio
	*/
	//-----Text Button----
	UIButton *textButton = [UIButton buttonWithType: UIButtonTypeRoundedRect];
	textButton.frame = CGRectMake(105,0,100,100);
	[textButton setTitle:@"Price" forState:UIControlStateNormal];
	...
	[choices release];
	appDelegate.coinController.editSelect = blarg;
	[blarg release];
	
	
	[appDelegate.coinController.editSelect 
	 presentPopoverFromRect:appDelegate.coinController.currentButton.frame inView:self.view
...
- (IBAction)btnShowAudio:(id)sender{ //Makes two views popup for single button. 
	...
	[sounds release];	   
	appDelegate.coinController.audioController = audioList;
	[audioList release];
	...
	[appDelegate.coinController.audioController 
	 presentPopoverFromRect:appDelegate.coinController.currentButton.frame inView:self.view
	 permittedArrowDirections:UIPopoverArrowDirectionAny 
	 animated:YES]; 
...
- (IBAction)btnShowImages:(id)sender{ //Makes two views popup for single button. 
	...
	[images release];		
	appDelegate.coinController.imageController = imageList;	
	[imageList release];
	
	//Properties of the popover views. Order matters as it acts like a stack. i.e. since the image controller was setup last, it will be the first displayed.
	
	[appDelegate.coinController.imageController 
	 presentPopoverFromRect:appDelegate.coinController.currentButton.frame inView:self.view
	 permittedArrowDirections:UIPopoverArrowDirectionAny 
	 animated:YES];	
...
- (IBAction)btnChangeText:(id)sender{
	//text alertview
	selectAlert = 3;
	UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Please Enter Price\n(using $0.00)" message:@""
												   delegate:self cancelButtonTitle:@"Cancel"  otherButtonTitles:@"Submit", nil];
